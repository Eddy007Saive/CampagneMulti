...
export function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setErrors({ submit: result.error || 'Inscription échouée' });
      }
    } catch (error) {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (!popup) {
        setErrors({ submit: 'Impossible d\'ouvrir la fenêtre d\'inscription. Veuillez autoriser les popups.' });
      ...
      checkInterval = setInterval(() => {
        if (popup && popup.closed && !isProcessing) {
          ...
          window.removeEventListener('message', handleMessage);
          setIsGoogleLoading(false);
          setErrors({ submit: 'Inscription annulée' });
        }
      }, 500);
        ...
        <div className="backdrop-blur-xl bg-bleu-fonce/40 border border-primary-500/30 rounded-3xl shadow-2xl hover:border-bleu-neon/50 transition-all duration-500 overflow-hidden">
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="w-full lg:w-1/2 relative group">
              <div className="relative h-[400px] lg:h-full min-h-[600px]">
                {/* Video */}
                <video
                  autoPlay
                  loop
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-6">
                  <div className="backdrop-blur-sm bg-bleu-fonce/50 rounded-2xl p-4 border border-bleu-neon/30">
                    <Typography variant="h4" className="text-blanc-pur font-bold mb-2">
                      Rejoignez l'innovation
                    </Typography>
                    <Typography className="text-gray-300 mb-3 text-sm">
                      "Prospectra a transformé notre façon de travailler."
                    </Typography>
              ...
              <div className="text-center mb-6">
                <div className="inline-block mb-3">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-primary flex items-center justify-center shadow-neon-gradient animate-glow">
                    <User className="w-7 h-7 text-blanc-pur" />
                  </div>
                </div>
                <Typography
                  variant="h2"
                  className="font-bold mb-2 text-transparent bg-clip-text bg-gradient-primary text-3xl"
                >
                  Créer un compte
                </Typography>
                <Typography
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative group">
                    <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-bleu-neon transition-colors" />
                    <Input
                      type="text"
                      name="name"
                      size="md"
                      ...
                      error={!!errors.name}
                      className="!pl-10 !border-primary-500/50 focus:!border-bleu-neon bg-bleu-fonce/50 text-blanc-pur placeholder:text-gray-500 rounded-xl transition-all duration-300 hover:bg-bleu-fonce/70"
                      labelProps={{
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative group">
                    ...
                    <Input
                      ...
                      className="!pl-10 !pr-10 !border-primary-500/50 focus:!border-bleu-neon bg-bleu-fonce/50 text-blanc-pur placeholder:text-gray-500 rounded-xl transition-all duration-300 hover:bg-bleu-fonce/70"
                      labelProps={{
                        className: "before:content-none after:content-none",
                      }}
                    />
                ...
                <div className="space-y-1">
                  <Checkbox
                    ...
                    label={
                      <Typography
                        variant="small"
                        className="font-medium text-gray-300 flex items-start"
                      >
                        J'accepte les{' '}
                        <a
                          href="#"
                          className="text-bleu-neon hover:text-violet-plasma transition-colors ml-1 underline"
                        >
                  ...
                  />
                  {errors.terms && (
                    <Typography variant="small" color="red" className="ml-1 text-[10px]">
                      {errors.terms}
                    </Typography>
                  )}
                </div>
                ...
                <Button
                  type="submit"
                  className="w-full bg-gradient-primary hover:shadow-neon-gradient transition-all duration-300 rounded-xl py-2.5 group relative overflow-hidden"
                  disabled={isSubmitting || loading}
                >
                  <span className="relative z-10 flex items-center justify-center gap-2 text-sm">
                    {isSubmitting || loading ? 'Création en cours...' : 'Créer mon compte'}
                ...
                <div className="relative my-4">
                  ...
                  <div className="relative flex justify-center text-xs">
                    <span className="px-3 bg-bleu-fonce/40 text-gray-400">Ou</span>
                  </div>
                </div>
                ...
                >
                  <span className="flex items-center justify-center gap-2">
                    ...
                    <span className="group-hover:translate-x-1 transition-transform text-xs">
                      {isGoogleLoading ? 'Inscription...' : 'Google'}
                    </span>
                  </span>
                </Button>
                ...
                <Typography
                  variant="small"
                  className="text-center text-gray-400 mt-3 text-xs"
                >
                  Déjà un compte ?{' '}
                  <Link
                    to="/auth/sign-in"
