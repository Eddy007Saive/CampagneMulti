...
const handleGoogleLogin = async () => {
  console.log('🚀 [GOOGLE LOGIN] Début du processus');
  setIsGoogleLoading(true);
  setErrors({});

  // ✅ OUVRIR LE POPUP IMMÉDIATEMENT (avant tout code async)
  const width = 500;
  const height = 600;
  const left = window.screenX + (window.outerWidth - width) / 2;
  const top = window.screenY + (window.outerHeight - height) / 2;

  const popupUrl = `${import.meta.env.VITE_BASE_URL}/google`;
  ...
  console.log('🪟 [POPUP] État après ouverture:', {
    popup: !!popup,
    closed: popup?.closed,
  ...
  if (!popup || popup.closed || typeof popup.closed === 'undefined') {
    ...
    setErrors({ 
      submit: 'Popup bloqué ! Veuillez autoriser les popups pour ce site dans les paramètres de votre navigateur.' 
    });
  ...
  const cleanup = () => {
    console.log('🧹 [CLEANUP] Nettoyage des événements');
    if (checkInterval) {
  ...
  const handleMessage = async (event) => {
    ...
    console.log('🔐 [SECURITY] Vérification origine:', {
    ...
    if (!allowedOrigins.includes(event.origin)) {
      console.warn('⚠️ [SECURITY] Message rejeté - origine non autorisée');
      return;
    }

    // Marquer comme en cours de traitement IMMÉDIATEMENT
    console.log('⚙️ [PROCESSING] Début du traitement');
    isProcessing = true;
    cleanup();

    const { success, data, error } = event.data;

    console.log('📦 [DATA] Contenu du message:', {
      success,
      hasData: !!data,
      error,
    ...
    if (success && data) {
      console.log('✅ [SUCCESS] Données reçues, appel loginWithGoogle...');
      try {
        const result = await loginWithGoogle(data);
        console.log('🔄 [LOGIN] Résultat loginWithGoogle:', result);

        // Fermer le popup APRÈS le traitement
        setTimeout(() => {
        ...
        if (result.success) {
          console.log('🎉 [REDIRECT] Navigation vers /dashboard/home');
          navigate('/dashboard/home');
        } else {
          console.error('❌ [LOGIN] Échec:', result.error);
          setErrors({ submit: result.error || 'Erreur lors de la connexion' });
        }
      } catch (err) {
        console.error('💥 [ERROR] Exception dans loginWithGoogle:', err);
      ...
      console.error('❌ [FAIL] Échec authentification:', error);
  ...
  console.log('👂 [LISTENER] Ajout du listener de messages');
  window.addEventListener('message', handleMessage);

  // Timeout de 60 secondes pour l'authentification
  timeoutId = setTimeout(() => {
    if (!isProcessing) {
      console.warn('⏱️ [TIMEOUT] Délai dépassé (60s)');
      cleanup();
      if (popup && !popup.closed) {
        popup.close();
      }
      setIsGoogleLoading(false);
      setErrors({ submit: 'Délai d\'authentification dépassé' });
    }
  }, 60000);

  // Vérifier si le popup est fermé manuellement (intervalle plus long)
  let checkCount = 0;
  checkInterval = setInterval(() => {
    checkCount++;
    try {
      const isClosed = popup && popup.closed;
      if (checkCount % 5 === 0) { // Log tous les 5 checks (5 secondes)
        console.log(`🔍 [CHECK #${checkCount}] État popup:`, {
          isClosed,
          isProcessing,
          messageCount
        });
      ...
      if (isClosed && !isProcessing) {
        console.warn('🚫 [CANCELED] Popup fermé par l\'utilisateur');
        cleanup();
        setIsGoogleLoading(false);
        setErrors({ submit: 'Connexion annulée' });
      }
    } catch (e) {
      console.error('⚠️ [CHECK] Erreur lors de la vérification:', e);
    }
  }, 1000);

  console.log('✅ [SETUP] Configuration terminée, attente du message...');
};
  return (
